...

{
	...
	if (lower_dev) {
		...
		//if (container_shift_ids(vol->container, lower_dir, lower_dir, NULL)) {
		//	ERROR_ERRNO("Could not mount lower_dir '%s' idmapped", lower_dir);
		//	goto error;
		//}
		//DEBUG("Successfully idmapped lower_dir '%s'", lower_dir);

	} else {
		lower_dir = mem_strdup(target_dir);
	}
	//	if (strcmp(upper_dev, "tmpfs") != 0) {
	//if (container_shift_ids(vol->container, overlayfs_mount_dir, overlayfs_mount_dir,
	//				NULL)) {
	//	ERROR_ERRNO("Could not mount '%s' idmapped", overlayfs_mount_dir);
	//	goto error;
	//}
	//DEBUG("Successfully idmapped %s", overlayfs_mount_dir);
	//	} else {
	//		if (chown(upper_dir, uid, uid) < 0) {
	//			ERROR_ERRNO("Could not chown dir '%s' to (%d:%d)", upper_dir, uid, uid);
	//			goto error;
	//		}
	//		if (chown(work_dir, uid, uid) < 0) {
	//			ERROR_ERRNO("Could not chown dir '%s' to (%d:%d)", work_dir, uid, uid);
	//			goto error;
	//		}
	//	}
	//
...
c_vol_mount_dev(c_vol_t *vol)
{
	...
	int ret = -1;
	int loop_fd = 0;
	char *dev_loopdev = NULL;
	char *dev_img_path = NULL;
	...
	dev_img_path = mem_printf("%s/dev.img", container_get_images_dir(vol->container));
	if ((ret = c_vol_create_image_empty(dev_img_path, NULL, 10)) < 0)
		goto error;

	if (c_vol_format_image(dev_img_path, "ext4") < 0) {
		goto error;
	}

	dev_loopdev = loopdev_create_new(&loop_fd, dev_img_path, 0, 0);
	IF_NULL_GOTO(dev_loopdev, error);

	if ((ret = mount(dev_loopdev, dev_mnt, "ext4", MS_RELATIME | MS_NOSUID, NULL)) < 0) {
...
error:
	if (loop_fd > 0)
		close(loop_fd);
	mem_free0(dev_mnt);
	mem_free0(dev_img_path);
	mem_free0(dev_loopdev);
	return ret;
}
